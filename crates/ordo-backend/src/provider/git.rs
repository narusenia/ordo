...
use crate::lua::{LuaContext, LuaRunner};
    ...
    match compiler::detect_compiler() {
        Some(info) => {
            ...
            let cc = match info.kind {
                ordo_core::manifest::CompilerKind::Gcc => "gcc".to_string(),
                ordo_core::manifest::CompilerKind::Msvc => "cl".to_string(),
                _ => "clang".to_string(),
            };
            let cxx = match info.kind {
                ordo_core::manifest::CompilerKind::Gcc => "g++".to_string(),
                ordo_core::manifest::CompilerKind::Msvc => "cl".to_string(),
                _ => "clang++".to_string(),
            };
            (cc, cxx, id)
