...
use ordo_cli::context::Context;
use ordo_cli::{Cli, ColorMode, Command, ProjectLang};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Command::New {
            ...
            match name {
                Some(name) => {
                    ...
                    ordo_cli::new::run(&cwd, &name, lib, lang, no_git, &ctx)?;
                }
                None => {
                    ordo_cli::new::run_interactive(&cwd, no_git, &ctx)?;
                }
            }
        }
        Command::Init => {
            let cwd = std::env::current_dir().into_diagnostic()?;
            ordo_cli::init::run(&cwd, &ctx)?;
        }
        Command::Build {
            release,
            profile,
            ...
            let opts = ordo_cli::build::BuildOptions {
                release,
                profile,
                jobs,
                target,
        ...
        Command::Test {
            ...
            let opts = ordo_cli::test::TestOptions {
                filter,
                jobs,
                release,
                profile,
        ...
        Command::Clean { cache, package } => {
            ordo_cli::clean::run(cache, package.as_deref(), &ctx)?;
        }
        Command::Fmt { check, package } => {
            ordo_cli::fmt::run(check, package.as_deref(), &ctx)?;
        }
        Command::Lint { fix, package } => {
            ordo_cli::lint::run(fix, package.as_deref(), &ctx)?;
        }
        Command::Watch { .. } => {
        ...
        Command::Update { name } => {
            let cwd = std::env::current_dir().into_diagnostic()?;
            ordo_cli::update::run(&cwd, name.as_deref(), &ctx)?;
        }
        Command::Tree { .. } => {
            let cwd = std::env::current_dir().into_diagnostic()?;
            ordo_cli::tree::run(&cwd, &ctx)?;
        }
        Command::Install { .. } => {
        ...
        Command::RunScript { name } => {
            ordo_cli::run_script::run(&name, &ctx)?;
        }
        Command::SelfCmd { .. } => {
