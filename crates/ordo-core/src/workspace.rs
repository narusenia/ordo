...
use crate::manifest::{DependencySource, Language, Manifest, Toolchain, WorkspaceConfig};
...
pub struct EffectiveLanguage {
    pub c: Option<crate::manifest::CStandard>,
    pub cpp: Option<crate::manifest::CppStandard>,
}

#[derive(Debug)]
pub struct EffectiveToolchain {
    pub compiler: Option<crate::manifest::CompilerKind>,
    pub linker: Option<crate::manifest::LinkerKind>,
}
...
mod tests {
    ...
    fn toolchain_inheritance() {
        ...
        assert_eq!(core_tc.compiler, Some(crate::manifest::CompilerKind::Clang));
        assert_eq!(core_tc.linker, Some(crate::manifest::LinkerKind::Lld));

        let special = ws.find_member("special").unwrap();
        let special_tc = special.effective_toolchain(root_tc);
        assert_eq!(
            special_tc.compiler,
            Some(crate::manifest::CompilerKind::Gcc)
        );
        assert_eq!(special_tc.linker, Some(crate::manifest::LinkerKind::Lld));

        let core_lang = core.effective_language(root_lang);
        assert_eq!(core_lang.cpp, Some(crate::manifest::CppStandard::Cpp23));
    }
