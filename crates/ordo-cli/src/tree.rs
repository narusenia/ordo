...
use ordo_backend::provider::{FetchedDep, Provider};
use ordo_core::manifest::{DependencySource, Manifest, ProviderKind};
use std::path::Path;
...
pub fn run(dir: &Path, ctx: &Context) -> Result<()> {
    ...
    let all_deps: Vec<(&String, &ordo_core::manifest::DependencySpec, bool)> = manifest
        .dependencies
        .iter()
        .map(|(n, s)| (n, s, false))
